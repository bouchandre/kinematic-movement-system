...
namespace KinematicMovementSystem
{
	...
	public interface ICharacterMotorController
	{
		/// <summary>
...
#pragma warning restore 0649
		...
		/// </summary>
		[System.NonSerialized]
		CharacterTransientGroundingReport _lastGroundingStatus = new CharacterTransientGroundingReport();
		...
		/// Contains the current grounding information
		/// </summary>
		public GroundReport Ground => _groundProperties;
		[System.NonSerialized]
		GroundReport _groundProperties = new GroundReport();
		...
		public const float MinimumGroundProbingDistance = 0.01f;
...
#pragma warning restore 0414
		...
		{
			KinematicSystemManager.UnregisterCharacterMotor(this);
		}
		...
		public void UpdatePhase1(float deltaTime)
		{
			...
			_velocityReport.LateInput = _velocityReport.Input;
			...
			_groundingStatus.GroundNormal = _characterUp;

			_groundProperties.FoundAnyGround = false;
			_groundProperties.StableOnGround = false;
			_groundProperties.IsOnLedge = false;
			_groundProperties.Normal = _characterUp;
			_groundProperties.CapsuleNormal = _characterUp;
			...
			if (_solveGrounding)
			{
				...
				else
				{
					...
					if (!_lastGroundingStatus.IsStableOnGround && _groundingStatus.IsStableOnGround)
					{
						...
						_baseVelocity = GetDirectionTangentToSurface(_baseVelocity, _groundingStatus.GroundNormal) * _baseVelocity.magnitude;
					}
				}
			}
			...
			if (InteractiveRigidbodyHandling)
			{
				...
				else
				{
					...
					{
						Rigidbody interactiveRigidbody = GetInteractiveRigidbody(_groundingStatus.GroundCollider);
				...
				{
					_baseVelocity -= Vector3.ProjectOnPlane(_attachedRigidbodyVelocity, _characterUp);
				}
		...
		public void UpdatePhase2(float deltaTime)
		{
			...
			_velocityReport.Input = _baseVelocity;
...
#if UNITY_EDITOR
			_capsule.hideFlags = HideFlags.HideInInspector;
		...
		void ProbeGround(ref Vector3 probingPosition, Quaternion atRotation, float probingDistance, ref CharacterGroundingReport groundingReport)
		{
			...
			while (groundProbeDistanceRemaining > 0 && (groundSweepsMade <= MaxGroundingSweepIterations) && !groundSweepingIsOver)
			{
						...
						out groundSweepHit)) // hit
				{
					...
					EvaluateHitStability(groundSweepHit.collider, groundSweepHit.normal, groundSweepHit.point, targetPosition, _transientRotation, _baseVelocity, ref groundHitStabilityReport);
					...
					if (groundHitStabilityReport.IsStable)
					{
						...
						groundingReport.SnappingPrevented = !IsStableWithSpecialCases(ref groundHitStabilityReport, _baseVelocity);
						...
						_groundProperties.StableOnGround = true;
						...
						for (int i = 0; i < numOverlaps; i++)
						{
								...
								out float resolutionDistance))
							{
								...
								if (dotProduct < 0f && dotProduct < mostObstructingOverlapNormalDotProduct)
								{
									...
									closestSweepHitPoint = tmpMovedPosition + (_transientRotation * _characterTransformToCapsuleCenter) + (resolutionDirection * resolutionDistance);
		...
		Vector3 GetObstructionNormal(Vector3 hitNormal, bool stableOnHit)
		{
			...
			{
				Vector3 obstructionLeftAlongGround = Vector3.Cross(_groundingStatus.GroundNormal, obstructionNormal).normalized;
		...
		/// </summary>
		void InternalHandleVelocityProjection(bool stableOnHit, Vector3 hitNormal, Vector3 obstructionNormal, Vector3 originalDirection, ref MovementSweepState sweepState, bool previousHitIsStable,
			Vector3 previousVelocity, Vector3 previousObstructionNormal, ref Vector3 transientVelocity, ref float remainingMovementMagnitude, ref Vector3 remainingMovementDirection)
		{
		...
		void EvaluateCrease(Vector3 currentCharacterVelocity, Vector3 previousCharacterVelocity, Vector3 currentHitNormal, Vector3 previousHitNormal,
			bool currentHitIsStable, bool previousHitIsStable, bool characterIsStable, out bool isValidCrease, out Vector3 creaseDirection)
		{
			isValidCrease = false;
		...
		void ComputeCollisionResolutionForHitBody(Vector3 hitNormal, Vector3 characterVelocity, Vector3 bodyVelocity,
			float characterToBodyMassRatio, out Vector3 velocityChangeOnCharacter, out Vector3 velocityChangeOnBody)
		{
		...
		bool CheckIfColliderValidForCollisions(Collider coll)
		{
			// Ignore self
			if (coll == _capsule)
			{
		...
		/// </summary>
		void GetVelocityFromRigidbodyMovement(Rigidbody interactiveRigidbody, Vector3 atPoint, float deltaTime, out Vector3 linearVelocity, out Vector3 angularVelocity)
		{
			if (deltaTime > 0f)
			{
